...
for (let i = 0; i < allArticle.length; i++) {
  allArticle[i].addEventListener('click', function(event) {
    let pictureClicked = event.currentTarget.querySelector('h2').innerText;
    for (let i = 0; i < picArray.length; i++) {
      if (picArray[i].title === pictureClicked) {
        dialogue.querySelector('img').src = `${picArray[i].image.large}`;
        dialogue.querySelector('img').alt = `${picArray[i].title}`;
      }
    }
    dialogue.showModal();
  })
...
close.addEventListener('click', function(){dialogue.close()})
